...
def snap2seg(segment, point):
    ...
    def e(x): return segEqn(
        segment['type'], segment['P1'], segment['P3'], x, segment['P2'])
    return e(x)
...
def convert(value, dim=[1, -2, 0], unitFrom=['SI', 1, 1, 'C'], unitTo=['ENG', 1, 1, 'C']):
    ...
    return value*(lenRatio*lenFac)**dim[1]*(forRatio*forFac)**dim[0]*(tempFac)**dim[2]
...
def quadNodes(curve, fac=0.9995, nmax=8):
    ...
    lmin = fac*s
    xs = array([0, 0.5*l])
    def e(x): return quadEqn(P1, P3, P2, x)
    n = 1
    pts = apply(e, xs)
    ...
    pts = apply(e, xs)[1:-1]
    return apply(lambda x: {'type': 'Node',
                            'location': x,
                            'settlement': zeros(6),
                            'normal': array([0, 0, 1]),
                            'class': 'support'}, pts)
...
def arcNodes(curve, fac=0.9995, nmax=8):
    ...
    lmin = fac*s
    xs = array([0, 0.5*l])
    def e(x): return arcEqn(P1, P3, P2, x)
    n = 1
    pts = apply(e, xs)
    while (l <= lmin and n <= nmax):
        mids = apply(
...
def breakLoad(load, points, segments):
    ...
    def X(x): return dot(x-ps['P1'], unit(ps['P3']-ps['P1']))
    xSegs = apply(lambda x: X(x['P1']), segments)

    psIndex = apply(lambda y: int(
        max(((points[y]-xSegs) >= 0).nonzero()[0])), arange(points.size-1))

    coords = apply(lambda x: segEqn(
        ps['type'], ps['P1'], ps['P3'], x, ps['P2']), points)

    udls = apply(lambda x: {'degree': 0,
                            'parentSegment': segments[psIndex[x]],
                            'P1': coords[x],
                            'P3': coords[x+1],
                            'normal': load['normal'],
                            'peak': a*(points[x]-xStart)**n,
                            'class': 'load'}, arange(points.size-1))

    dls = apply(lambda x: {'degree': n,
                           'parentSegment': segments[psIndex[x]],
                           'P1': coords[x],
                           'P3': coords[x+1],
                           'normal': load['normal'],
                           'peak': a*((points[x+1]-xStart)**n-(points[x]-xStart)**n),
                           'class': 'load'}, arange(points.size-1))
...
def breakSegment(seg, points, loads=array([])):
    ...
    xs = unique(
        sort(apply(lambda loc: dot(loc-seg['P1'], unit(seg['P3']-seg['P1'])), points)))
    ...
    # secTheta = lambda x: 1
    segments = apply(lambda x: {'type': seg['type'],
                                'P1': e(xs[x]),
                                'P3': e(xs[x+1]),
                                'P2': e(mids[x]),
                                ...
                                'class': 'segment',
                                'parent': seg.copy()}, arange(xs.size-1))
    ...
    if (pointLoads.size != 0):
        ts = apply(lambda x: dot(x['P1']-seg['P1'],
                   unit(seg['P3']-seg['P1'])), pointLoads)
        index = apply(lambda x: max(
            hstack((0, *(around(x-xs[:-1], 10) > 0).nonzero()[0]))), ts)
        apply(changeParent, apply(
            lambda x: [pointLoads[x], segments[index[x]]], arange(index.size)))

    if (distLoads.size != 0):
        ts = apply(lambda x: array([dot(x['P1']-seg['P1'], unit(seg['P3']-seg['P1'])),
                                    dot(x['P3']-seg['P1'], unit(seg['P3']-seg['P1']))]), distLoads)
        index = apply(lambda ts: array([min((around(xs-ts[0], 10) > 0).nonzero()[0])-1,
                                        min((around(xs-ts[1], 10) >= 0).nonzero()[0])-1]), ts)
        ...
        if (goodIndex.size != 0):
            ...
            apply(changeParent, apply(
                lambda x: [goodDists[x], segments[goodIndex[x]]], arange(goodIndex.size)))

        if (badIndex.size != 0):
            badTs = ts[badIndex]
            badXs = apply(lambda x:
                          unique(sort(hstack((x, xs[apply(lambda xs: xs >= x[0] and xs <= x[1], xs)])))), badTs)
            badDists = apply(lambda x: breakLoad(
                badDists[x], badXs[x], segments), arange(badIndex.size)).flatten()
...
def childOf(seg, simFrame):
    ...
    return segments[array([identicalObjects(seg, x['parent'] if x['parent'] != None else x) for x in segments]).nonzero()[0]]
...
def simplify(str, simFac=0.9995):
    ...
    segNodes = apply(lambda x: [{'type': 'Node', 'location': x['P1'], 'settlement': zeros(6), 'normal': array([0, 0, 1]), 'class': 'support'},
                                {'type': 'Node', 'location': x['P3'], 'settlement': zeros(6), 'normal': array([0, 0, 1]), 'class': 'support'}], segments)
    ...
    def checkLoc(loc, seg):
        ...
        if norm(loc-A) < norm(B-A) and norm(loc-B) < norm(B-A):
            return tolClose(loc, segEqn(seg['type'], seg['P1'], seg['P3'], dot(loc-seg['P1'], unit(seg['P3']-seg['P1'])), seg['P2']))
        else:
            return False
    ...
    check = apply(lambda sp:
                  apply(lambda seg:
                        checkLoc(sp['location'], seg),
                        segments).any(), supports)
    ...
    check2 = apply(lambda seg: apply(
        lambda loc: checkLoc(loc, seg), locations), segments)
    breakMatrix = apply(lambda x: apply(
        lambda y: y['location'], x), apply(lambda x: badSps[x], check2))

    def segLoads(seg): return loads[apply(
        lambda x: identicalObjects(x['parentSegment'], seg), loads)]

    elements = apply(lambda x: breakSegment(
        segments[x], breakMatrix[x], segLoads(segments[x])), arange(segments.size))
...
def generate3d(buildingDimension=array([2, 3, 4]),
    ...
    firstFloorXbeams = apply(lambda yy: apply(
        lambda xx: array([xx, yy]), x), y).reshape((noOfPoints, 2))
    firstFloorYbeams = apply(lambda yy: apply(
        lambda xx: array([yy, xx]), y), x).reshape((noOfPoints, 2))

    notX = arange(noOflongs, (noOfwides+1)*noOflongs +
                  1, noOflongs)+arange(noOfwides+1)
    indexX = arange(noOfPoints)
    indexX = indexX[apply(lambda x: x not in notX, indexX)]

    notY = arange(noOfwides, (noOflongs+1)*noOfwides +
                  1, noOfwides)+arange(noOflongs+1)
    indexY = arange(noOfPoints)
    indexY = indexY[apply(lambda x: x not in notY, indexY)]

    firstFloorX2d = apply(lambda x: array(
        [firstFloorXbeams[x,], firstFloorXbeams[x+1,]]).flatten(), indexX)
    firstFloorY2d = apply(lambda x: array(
        [firstFloorYbeams[x,], firstFloorYbeams[x+1,]]).flatten(), indexY)
    X2d = tile(firstFloorX2d, (noOfFloors, 1))
    Y2d = tile(firstFloorY2d, (noOfFloors, 1))

    heightsX = repeat(arange(0, noOfFloors*height, height),
                      firstFloorX2d.shape[0]).reshape((noOfFloors*firstFloorX2d.shape[0], 1))
    heightsY = repeat(arange(0, noOfFloors*height, height),
                      firstFloorY2d.shape[0]).reshape((noOfFloors*firstFloorY2d.shape[0], 1))
    X3d = hstack((X2d[:, 0:2], heightsX, X2d[:, 2:4],
                 heightsX))+tile(origin, 2)
    Y3d = hstack((Y2d[:, 0:2], heightsY, Y2d[:, 2:4],
                 heightsY))+tile(origin, 2)
    ...
    Z3d = apply(lambda x: array(
        [Zpoints[x,], Zpoints[x+1,]]).flatten(), indexZ)+tile(origin, 2)

    if (add):
        X3d = X3d[noOflongs*(noOfwides+1):X3d.shape[0]]
        Y3d = Y3d[noOfwides*(noOflongs+1):Y3d.shape[0]]

    beamsX = apply(lambda x: '"'+str({'type': 'line',
                                      'P1': X3d[x][0:3],
                                      'P3': X3d[x][3:6],
                                      ...
                                      'class': 'segment'})+'"', arange(X3d.shape[0]))

    beamsY = apply(lambda x: '"'+str({'type': 'line',
                                      'P1': Y3d[x][0:3],
                                      'P3': Y3d[x][3:6],
                                      ...
                                      'class': 'segment'})+'"', arange(Y3d.shape[0]))

    column = apply(lambda x: '"'+str({'type': 'line',
                                      'P1': Z3d[x][0:3],
                                      'P3': Z3d[x][3:6],
                                      ...
                                      'axisVector': array([1, 0, 0]),
                                      'class': 'segment'})+'"', arange(Z3d.shape[0]))
    ...
    XsegmentsList = apply(lambda x: array(
        [x, namesX[x], 'segment', beamsX[x], 'True']), arange(X3d.shape[0]))
    YsegmentsList = apply(lambda x: array(
        [x+X3d.shape[0], namesY[x], 'segment', beamsY[x], 'True']), arange(Y3d.shape[0]))
    ZsegmentsList = apply(lambda x: array(
        [x+X3d.shape[0]+Y3d.shape[0], namesZ[x], 'segment', column[x], 'True']), arange(Z3d.shape[0]))

    segmentsList = vstack((array(
        ['', 'Name', 'Class', 'RObject', 'Flag']), ZsegmentsList, YsegmentsList, XsegmentsList))

    if (support):
        supports = apply(lambda x: '"'+str({'type': 'Fixed',
                                            'location': array([firstFloorXbeams[x][0], firstFloorXbeams[x][1], 0]),
                                            'settlement': zeros(6),
                                            'normal': array([0, 0, 1]),
                                            'class': 'support'})+'"', arange(noOfPoints))
        supportList = apply(lambda x: array(
            [x+X3d.shape[0]+Y3d.shape[0]+Z3d.shape[0], 'Fixed-'+str(x), 'support', supports[x], 'True']), arange(noOfPoints))
...
def generate2d(frameDimension=array([2, 4]),
    ...
    beamsCoord1 = apply(lambda h: apply(lambda x: array(
        [0, x, h*height]), y[h, arange(noOfwides)]), arange(noOfFloors))
    beamsCoord1 = beamsCoord1.flatten().reshape((int(beamsCoord1.size/3), 3))+origin
    beamsCoord2 = apply(lambda h: apply(lambda x: array(
        [0, x, h*height]), y[h, linspace(1, noOfwides, noOfwides, dtype=int)]), arange(noOfFloors))
    beamsCoord2 = beamsCoord2.flatten().reshape((int(beamsCoord2.size/3), 3))+origin
    beamsCoord = hstack((beamsCoord1, beamsCoord2))

    z = tile(linspace(0, (noOfFloors-1)*height, noOfFloors), (noOfwides+1, 1))
    columnsCoord1 = apply(lambda h: apply(lambda x: array(
        [0, h*breadth, x]), z[h, arange(noOfFloors-1)]), arange(noOfwides+1))
    columnsCoord1 = columnsCoord1.flatten().reshape(
        (int(columnsCoord1.size/3), 3))+origin
    columnsCoord2 = apply(lambda h: apply(lambda x: array(
        [0, h*breadth, x]), z[h, linspace(1, noOfFloors-1, noOfFloors-1, dtype=int)]), arange(noOfwides+1))
    ...
    if (support):
        supports = apply(lambda x: '"'+str({'type': 'Fixed',
                                            ...
                                            'settlement': zeros(6),
                                            'normal': array([0, 0, 1]),
                                            'class': 'support'})+'"', arange(noOfwides))
        supports = hstack((supports, '"'+str({'type': 'Fixed',
                                              'location': beamsCoord[noOfwides-1][0:3]+array([0, breadth, 0]),
                                             'settlement': zeros(6),
                                              'normal': array([0, 0, 1]),
                                              'class': 'support'})+'"'))
        supportList = apply(lambda x: array(
            [x+beamsCoord.shape[0]+columnsCoord.shape[0], 'Fixed-'+str(x), 'support', supports[x], 'True']), arange(noOfwides+1))

    if (add):
        beamsCoord = beamsCoord[noOfwides:beamsCoord.shape[0]]

    beams = apply(lambda x: '"'+str({'type': 'line',
                                     'P1': beamsCoord[x][0:3],
                                     'P3': beamsCoord[x][3:6],
                                     ...
                                     'axisVector': array([1, 0, 0]),
                                     'class': 'segment'})+'"', arange(beamsCoord.shape[0]))

    columns = apply(lambda x: '"'+str({'type': 'line',
                                       ...
                                       'shapeFactor': columnProp[6],
                                       'axisVector': array([1, 0, 0]),
                                       'class': 'segment'})+'"', arange(columnsCoord.shape[0]))
    ...
    beamsList = apply(lambda x: array(
        [x, namesBeam[x], 'segment', beams[x], 'True']), arange(beamsCoord.shape[0]))
    columnsList = apply(lambda x: array(
        [x+beamsCoord.shape[0], namesColumns[x], 'segment', columns[x], 'True']), arange(columnsCoord.shape[0]))

    segmentsList = vstack(
        (array(['', 'Name', 'Class', 'RObject', 'Flag']), columnsList, beamsList))
...
def transform(x, y, pan=array([0, 0]), zoom=1, rotate=0):
    rot = array([[cos(rotate), sin(rotate)],
