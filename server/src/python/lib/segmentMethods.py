...
def segPlotData(types, P1, P3, axisVector=array([1, 0, 0]), scale=2, P2=None, no=400):
    """
...
def arcPlotData2(P1, P3, P2, scale=1, no=50):
    ...
    def e(x): return arcEqn(hstack((0, P1)), hstack(
        (0, P3)), hstack((0, P2)), x, twoD=True)[1:3]
    d = apply(e, l)
    d = vstack((d, P3))
    return d/scale
...
def quadPlotData2(P1, P3, P2, scale=1, no=50):
    l = linspace(0, norm(P3-P1), no)
    def e(x): return quadEqn(hstack((0, P1)),
                             hstack((0, P3)), hstack((0, P2)), x)[1:3]
    d = apply(e, l)
    d = vstack((d, P3))
    return d/scale
...
def actionFun(line, frame, data, scale=1):
    ...
    L = lineTransformation(line['P1'], line['P3'], line['axisVector'], array(
        [0.0, 0.0, 1.0]), array([0.0, 0.0, 1.0]))[0:6, 0:6]
    ...
    tempLoads = loads[apply(lambda x: x['class'] in [
                            'tempLoad', 'misfitLoad'] and array_equal(x['P1'], line['P1']), loads)]
    if tempLoads.size:
        for x in tempLoads:
            if x['degree'] == -1:
                FA -= x['peak']*x['normal']
                ...
                MA -= x['peak']*x['normal']
    loads = loads[apply(lambda x: x['class'] == 'load' and identicalObjects(
        line, x['parentSegment']), loads)]
    index = apply(lambda x: x['degree'] >=
                  0 or not array_equal(x['P1'], line['P1']), loads)
    loads = loads[index] if index.size != 0 else array([{'degree': 0,
                                                       'parentSegment': line,
                                                        'P1': line['P1'],
                                                         'P3': line['P3'],
                                                         'normal': array([0, 0, 1]),
    ...
    def actionFunction(x):
        P = apply(lambda x: lineForceX(
            line['P1'], line['P3'], x['P1'], x['P3'], x['degree'], x['peak'], x['normal']), loads)
        mf = apply(lambda t: P[t](x), arange(P.size)
                   ) if P.size > 0 else array([zeros(3)])
        f = FA+apply(lambda x: sum(mf[:, x]), arange(3))

        M = apply(lambda x: lineMomentX(
            line['P1'], line['P3'], x['P1'], x['P3'], x['degree'], x['peak'], x['normal']), loads)
        mm = apply(lambda t: M[t](x), arange(M.size)
                   ) if M.size > 0 else array([zeros(3)])
...
def responseFun(line, frame, dataAct, dataRes, memLoc, shear, inextensible, scale=1, dispOnly=False, angleOnly=False):
    ...
    responses = lineTransformation(line['P1'], line['P3'], line['axisVector'], array(
        [0.0, 0.0, 1.0]), array([0.0, 0.0, 1.0]))[0:6, 0:6] @ dataRes[memLoc[Id, 0], 3:9]
    ...
    if not dispOnly and not angleOnly:
        def responseFunction(x):
            theta = apply(lambda x: lineAngleX(line['P1'], line['P3'], x['P1'], x['P3'], x['degree'], x['peak'], x['normal'],
                          line['youngsModulus'], line['shearModulus'], line['Iyy'], line['Izz'], line['J'], line['axisVector']), loads)
            ...
            thF = array([0,
                        ...
                        (0.5*actions[1]*x**2)/(line['youngsModulus']*line['Izz'])])
            thM = array([(actions[3]*x)/(line['shearModulus']*line['J']),
                        (actions[4]*x)/(line['youngsModulus']*line['Iyy']),
                        -(actions[5]*x)/(line['youngsModulus']*line['Izz'])])
            Theta = Theta + thF + thM

            delta = apply(lambda x: lineDisplaceX(line['P1'], line['P3'], x['P1'], x['P3'], x['degree'], x['peak'], x['normal'],
                          line['youngsModulus'], line['shearModulus'], line['Iyy'], line['Izz'], line['axisVector'], line['J'], A, k), loads)
            ...
            delF = array([-(actions[0]*x)/(line['youngsModulus']*A),
                          (thF[2]*x)/3 + (k[0]*actions[1]*x) /
                          (line['shearModulus']*A),
                          (thF[1]*x)/3 + (k[1]*actions[2]*x)/(line['shearModulus']*A)])
            delM = array([0,
                          (thM[2]*x)/2,
                          (thM[1]*x)/2])
...
def extractData(line, frame, fun, special=True, no=20, precision=15):
    if (line['type'] != 'line'):
        ...
        if special:
            ...
            logvec = apply(lambda x: identicalObjects(
                line, x['parentSegment']) and x['degree'] < 0, jumpLoad)
            if logvec.size != 0 and any(logvec):
                jumpLoad = jumpLoad[logvec]
                jumpXs1 = apply(lambda x: dot(
                    x['P1']-line['P1'], unit(line['P3']-line['P1'])), jumpLoad)
                jumpXs2 = apply(lambda x: dot(
                    x['P3']-line['P1'], unit(line['P3']-line['P1'])), jumpLoad)
                jumpXs = hstack((jumpXs1, 0.5*(jumpXs1+jumpXs2), jumpXs2, 0.5))
                step = 10**(-3)*norm(line['P3']-line['P1'])
                includeMe = apply(lambda x: array(
                    [x-step, x, x+step]), jumpXs).flatten()
        ...
        l = unique(
            hstack((linspace(0, norm(line['P3']-line['P1']), no+1), includeMe)))

    def normalize(x): return 0 if x <= 0 else (
        norm(line['P3']-line['P1']) if x >= norm(line['P3']-line['P1']) else x)
    l = apply(normalize, l)
...
def responsedata(line, frame, dataAct, dataRes, memLoc, shear=False, inextensible=False, scale=1, special=True, no=20, dispOnly=False, angleOnly=False, precision=15):
    fun = responseFun(line, frame, dataAct, dataRes, memLoc,
                      shear, inextensible, scale, dispOnly, angleOnly)
    data = extractData(line, frame, fun, special, no, precision)
    return data
...
def extractDataGeneral(line, simFrame, dataFun):
    ...
    xss = apply(lambda x: dot(x['P1']-line['P1'],
                unit(line['P3']-line['P1'])), children)
    xfs = apply(lambda x: dot(x['P3']-line['P1'],
                unit(line['P3']-line['P1'])), children)
...
def actionData(line, simFrame, rawData, no=20, special=True, scale=1):
    ...
    def dataFun(x): return actiondata(x, simFrame, rawData, scale, special, no)
    Data = extractDataGeneral(line, simFrame, dataFun)
    return Data


def responseData(line, frameData, shear=False, inextensible=False, no=20, special=True, scale=1, dispOnly=False, angleOnly=False):
    if line['type'] != 'line':
        no = 1

    def dataFun(x): return responsedata(x, frameData['simplified'], frameData['actionRaw'],
                                        frameData['responseRaw'], frameData['memLoc'], shear, inextensible, scale, special, no, dispOnly, angleOnly)
...
def trussActionData(segment, simTruss, actionRaw, scale=1):
    ...
    def af(x): return actionRaw[2*dictInArr(x, simTruss['segments']), -1]*scale
    def l(x): return norm(x['P3']-x['P1'])
    def data(x): return array([[0, af(x)], [l(x), af(x)]])
    datas = vstack(apply(data, children))
    ls = repeat(array([0, *apply(l, children)[:-1]]), 2)
    datas[:, 0] += ls
    return vstack(([0]*7, hstack((datas, zeros((len(datas), 5)))), [datas[-1, 0], *[0]*6], [0]*7))


def trussResponseData(segment, frameData, scale=1):
    simTruss, responseRaw, memLoc = frameData['simplified'], frameData['responseRaw'], frameData['memLoc']
    from lib.extensions import childOf
    children = childOf(segment, simTruss)
    def l(line): return norm(line['P3']-line['P1'])
    ...
    def data(line): return hstack(
        (vstack((0, l(line))), scale*responses(line)))
    datas = vstack(apply(data, children))
    ls = repeat(array([0, *apply(l, children)[:-1]]), 2)
    datas[:, 0] += ls
